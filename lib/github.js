...
function getComponentTagCounts( index, callback ) {
  ...
  getTags( components[ index ], function( err, tags ) {
    ...
    getComponentTagCounts( index + 1, function( err, counts ) {
      if ( err ) {
        return callback( err );
      }
      counts.push({
...
function getSuiteContributors( callback ) {
  getComponentContributors( 0, function( err, components ) {
    if ( err )
      return callback( err );
...
module.exports = {
  ...
  components: {
    ...
    tags: {
      counts: function( callback ) {
        getComponentTagCounts( 0, callback );
      }
    },
  ...
  suite: {
    ...
    commits: {
      count: function( callback ) {
        getCommitCounts( 0, function( err, components ) {
          if ( err )
            return callback( err );
          var count = 0;
          components.forEach( function( component ) {
    ...
    releases: {
      count: function( callback ) {
        getComponentTagCounts( 0, function( err, components ) {
          if ( err )
            return callback( err );
          var count = 0;
          components.forEach( function( component ) {
